...
std::any hw5::SimpleLangVisitor::visitFuncDecl(hw5::SimpleLangParser::FuncDeclContext *ctx) 
{
    ...
    std::vector<llvm::Type*> funcParamTypes(ctx->funcArgs()->ID().size(), llvm::Type::getInt64Ty(*ctxLLVM));
    llvm::FunctionType *funcType = llvm::FunctionType::get(llvm::Type::getInt64Ty(*ctxLLVM), funcParamTypes, false);
    ...
    for (size_t arg = 0; arg != ctx->funcArgs()->ID().size(); ++arg) {
        varsInFuncs[functionName][ctx->funcArgs()->ID(arg)->getText()] = builder->CreateAlloca(llvm::Type::getInt64Ty(*ctxLLVM), nullptr);
    ...
    builder->CreateRet(builder->CreateLoad(llvm::Type::getInt64Ty(*ctxLLVM), varsInFuncs[currFunc][ctx->ID()->getText()]));
    return "";
}
...
std::any hw5::SimpleLangVisitor::visitVarDecl(hw5::SimpleLangParser::VarDeclContext *ctx)
{
    ...
    llvm::Value *i = builder->CreateAlloca(llvm::Type::getInt64Ty(*ctxLLVM), nullptr);
    builder->SetInsertPoint(x);
...
std::any hw5::SimpleLangVisitor::visitFuncCall(hw5::SimpleLangParser::FuncCallContext *ctx)
{
    ...
    for (size_t arg = 0; arg != ctx->funcArgs()->ID().size(); ++arg) {
        args.push_back(builder->CreateLoad(llvm::Type::getInt64Ty(*ctxLLVM), varsInFuncs[currFunc][std::string(ctx->funcArgs()->ID(arg)->getText())]));
    }
...
std::any hw5::SimpleLangVisitor::visitCond_expr(hw5::SimpleLangParser::Cond_exprContext *ctx) {
    llvm::Value* loaded1 = builder->CreateLoad(llvm::Type::getInt64Ty(*ctxLLVM), varsInFuncs[currFunc][ctx->ID(0)->getText()]);
    llvm::Value* loaded2 = builder->CreateLoad(llvm::Type::getInt64Ty(*ctxLLVM), varsInFuncs[currFunc][ctx->ID(1)->getText()]);
    if (ctx->LESS()) {
