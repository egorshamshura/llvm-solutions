...
void IRGen::build(Binary& bin) {
  ...
  GlobalVariable* regFile = new GlobalVariable(*module, regFileType, false, GlobalValue::PrivateLinkage, 0, "regFile");
  ...
  FunctionCallee simPutPixelFunc = module->getOrInsertFunction("simPutPixel", int32x3FuncType);
  ...
  FunctionCallee dumpFunc = module->getOrInsertFunction("dump", dumpType);
  ...
  std::sort(vs.begin(), vs.end(),
            [](const std::pair<size_t, std::string>& a, const std::pair<size_t, std::string>& b) -> bool {
  ...
  std::unordered_map<std::string, FunctionCallee> FuncMap = {
      {"simPutPixelFunc", simPutPixelFunc},
      {   "simFlushFunc",    simFlushFunc},
